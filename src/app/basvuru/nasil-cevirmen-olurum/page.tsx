...
export default function OnsiteBasvuruPage() {
  ...
  const handleAddPage = useCallback(() => {
    ...
    const textarea = textareaRef.current; const selectionStart = textarea.selectionStart; const text = formData.mangaTest[currentTestIndex] || "";
    ...
    // --- TypeScript TYPE HATASI BURADA DÜZELTİLDİ ---
    const markers = Array.from(text.matchAll(pageMarkerRegex)).map((match: RegExpMatchArray) => ({ fullMatch: match[0], number: parseInt(match[1], 10), index: match.index! })); 
    
    let pageNumToInsertAfter = 0; for (const marker of markers) { if (marker.index < selectionStart) { pageNumToInsertAfter = marker.number; } else { break; }}
    ...
    let subsequentText = text.substring(selectionStart);
    subsequentText = subsequentText.replace(pageMarkerRegex, (match, p1) => { const renumberedNum = parseInt(p1, 10) + 1; return `<<<<<<<<<<<<<<<<<<<<<<<<<<<<< SAYFA ${String(renumberedNum).padStart(2, '0')} >>>>>>>>>>>>>>>>>>>>>>>>>>>>`; });
  ...
  // --- UYARI İÇİN GEREKSİZ DEPENDENCY KALDIRILDI ---
  ...
  useEffect(() => {
    if (saveState === 'saving') { const timer = setTimeout(() => setSaveState('saved'), 800); return () => clearTimeout(timer); }
    if(saveState === 'saved') { const timer = setTimeout(() => setSaveState('idle'), 1500); return () => clearTimeout(timer); }
  }, [saveState]);
      ...
      <main className="container mx-auto px-4 py-12 md:py-16">
        ...
        <AnimatePresence mode="wait">
            <motion.div key={currentStep} variants={stepVariants} initial="hidden" animate="visible" exit="exit" >
              ...
              {/* `finalNotes` adımı kaldırıldı, direkt teste geçiliyor. Notlar test içinde popup olarak var. */}
